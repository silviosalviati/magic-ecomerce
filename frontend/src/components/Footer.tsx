import { Link } from 'react-router-dom';

export function Footer() {
  return (
    <footer className="site-footer" id="sobre">
      <div className="footer-grid">
        <div className="footer-col footer-brand-col">
          <span className="footer-brand-name">MAGI.C</span>
          <p className="footer-tagline">Seu estilo, sua magia.</p>
          <div className="footer-social">
            <a
              href="https://wa.me/5511969707136"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="WhatsApp"
              className="footer-social-link"
            >
              <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z" />
                <path d="M12 0C5.373 0 0 5.373 0 12c0 2.123.554 4.118 1.528 5.855L.057 23.215a.75.75 0 0 0 .916.932l5.453-1.43A11.945 11.945 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.75a9.713 9.713 0 0 1-4.953-1.354l-.355-.211-3.676.964.982-3.585-.232-.37A9.713 9.713 0 0 1 2.25 12C2.25 6.615 6.615 2.25 12 2.25S21.75 6.615 21.75 12 17.385 21.75 12 21.75z" />
              </svg>
            </a>
            <a
              href="https://instagram.com/magi.c_br"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="Instagram"
              className="footer-social-link"
            >
              <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z" />
              </svg>
            </a>
          </div>
        </div>

        <div className="footer-col">
          <p className="footer-col-title">Loja</p>
          <nav className="footer-nav" aria-label="Links da loja">
            <a href="/#novidades">Novidades</a>
            <a href="/#feminino">Moda Feminina</a>
            <a href="/#masculino">Moda Masculina</a>
          </nav>
        </div>

        <div className="footer-col">
          <p className="footer-col-title">Atendimento</p>
          <nav className="footer-nav" aria-label="Links de atendimento">
            <a href="https://wa.me/5511969707136" target="_blank" rel="noopener noreferrer">
              WhatsApp
            </a>
            <a href="mailto:contato@vistamagic.com.br">contato@vistamagic.com.br</a>
            <Link to="/sobre">Sobre a Vista Magic</Link>
            <a href="https://wa.me/5511969707136" target="_blank" rel="noopener noreferrer">Trocas e Devoluções</a>
          </nav>
        </div>
      </div>

      <div className="footer-bottom">
        <span className="footer-copy">
          &copy; {new Date().getFullYear()} MAGI.C &mdash; Vista Magic. Todos os direitos reservados.
        </span>
        <div className="footer-payment-icons" aria-label="Formas de pagamento aceitas">
          <span className="payment-chip">PIX</span>
          <span className="payment-chip">Visa</span>
          <span className="payment-chip">Master</span>
          <span className="payment-chip">Boleto</span>
        </div>
      </div>
    </footer>
  );
}
